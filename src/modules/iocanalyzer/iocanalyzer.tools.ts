...
export class IocAnalyzerTools {
  ...
  async analyzeIoc(input: { ioc: string; iocType?: string }, context: ExecutionContext) {
  ...
  private analyzeIp(ip: string) {
    ...
    const reputationDb: Record<string, { score: number; reputation: string; indicators: string[] }> = {
  ...
  private analyzeHash(hash: string) {
    let riskScore = 0;
    const indicators: string[] = [];
    const mitigation: string[] = [];
