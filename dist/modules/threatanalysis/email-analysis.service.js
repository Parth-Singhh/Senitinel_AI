...
import { BRAND_NAMES, AUTHORITY_REGEX, CREDENTIAL_REGEX, DEFAULT_MITIGATION, DANGEROUS_ATTACHMENT_REGEX, FINANCIAL_REGEX, GRAMMAR_REGEX, HOMOGLYPH_REGEX, HTTP_REGEX, IP_URL_REGEX, LOGIN_PATH_REGEX, MIME_FILENAME_MISMATCH_PATTERNS, SHORTENER_REGEX, SUSPICIOUS_TLD_REGEX, THREAT_REGEX, URGENCY_REGEX, } from './email-rules';
import { extractDomain, extractUrlsFromText, normalizeText, uniqueStrings, } from './email-normalizer';
...
function getRecommendedAction(score, evidence) {
    const hasCritical = evidence.some((e) => e.severity === 'critical');
    if (score >= 91 || hasCritical)
        return 'block';
    if (score >= 71)
        return 'quarantine';
    if (score >= 46)
...
let EmailAnalysisService = class EmailAnalysisService {
    ...
    calibrateFinalScore(parts) {
        const weighted = parts.sender * 0.25 +
            parts.auth * 0.20 +
            parts.urls * 0.25 +
    ...
    analyze(input) {
        ...
        const confidence = clamp(0.55 + evidence.length * 0.04 + (input.auth?.dmarc === 'pass' ? 0.03 : 0) - (input.auth?.dmarc === 'fail' ? 0.08 : 0), 0.5, 0.99);
